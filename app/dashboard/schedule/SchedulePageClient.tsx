...
import { MapPin, Plus, CalendarIcon as CalendarIconLucide, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
export default function SchedulePageClient() {
  ...
  const getEventTypeColor = (type: string) => {
    switch (type) {
      case 'Interview': return 'bg-teal text-white';
      case 'Resume Review': return 'bg-yellow text-dark-text';
    ...
    <div className="container mx-auto py-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button className="bg-teal text-white hover:bg-teal-dark" onClick={() => setIsAddEventOpen(true)}>
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent>
            <div className="h-[600px] overflow-y-auto">
              {view === 'day' && (
                <div className="space-y-4">
                  {events
                    ...
                    .map((event) => (
                      <Card key={event.id} className="p-4 cursor-pointer hover:bg-gray-50" onClick={() => setSelectedEvent(event)}>
                  ...
                  {events.filter((event) => event.date.toDateString() === currentDate.toDateString()).length === 0 && (
                   ...
                   {Array.from({ length: 7 }).map((_, i) => {
                    ...
                    return (
                       <div key={i} className="py-2">
                         <h4 className="font-semibold text-sm mb-1">{day.toLocaleDateString('en-US', { weekday: 'long', day: 'numeric' })}</h4>
                         {dayEvents.length > 0 ? dayEvents.map(event => (
                           <Card key={event.id} className="p-2 mb-2 cursor-pointer hover:bg-gray-50 text-xs" onClick={() => setSelectedEvent(event)}>
                             <div className="flex justify-between">
                               <span>{event.title} ({event.time})</span>
                               <Badge variant="outline" className={getEventTypeColor(event.type) + " text-[10px] px-1 py-0"}>{event.type}</Badge>
                          </div>
                           </Card>
              ...
              {view === 'month' && (
                <p className="text-center text-gray-500 py-10">Month view: Display logic for events in a grid or list would go here.</p>
            )}
              </div>
          </CardContent>
        </Card>
          ...
          <DialogContent className="sm:max-w-[550px]">
            ...
            <div className="space-y-4 py-4">
              <div className="flex items-center">
                ...
                <span>
                  {selectedEvent.date.toLocaleDateString()} • {selectedEvent.time} ({selectedEvent.duration} min)
                </span>
              </div>
              <div className="flex items-center">
            ...
            <DialogFooter>
              <Button variant="destructive" onClick={() => { handleDeleteEvent(selectedEvent.id); setSelectedEvent(null); }}>Delete Event</Button>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                id="date" type="date" value={newEvent.date.toISOString().split('T')[0]}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setNewEvent({ ...newEvent, date: new Date(e.target.value) })}
              />
            </div>
            <div>
              <Label htmlFor="time">Time</Label>
              <Input
                id="time" type="time" value={newEvent.time}
            ...
            <div>
              ...
              <Input
                id="duration" type="number" value={newEvent.duration}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setNewEvent({ ...newEvent, duration: parseInt(e.target.value) || 0 })}
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                id="location" value={newEvent.location}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setNewEvent({ ...newEvent, location: e.target.value })}
              />
            </div>
             <div>
              <Label htmlFor="description">Description</Label>
              <Input
                id="description" value={newEvent.description}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setNewEvent({ ...newEvent, description: e.target.value })}
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAddEventOpen(false)}>Cancel</Button>
            <Button onClick={handleAddEvent} className="bg-teal text-white">Add Event</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
